 ...
 */
public class BBautOLATSTDisplayConfigController extends FormBasicController {
	 ...
	 */
	BBautOLATSTDisplayConfigController(UserRequest ureq, WindowControl wControl, ModuleConfiguration config, CourseEditorTreeNode node) {
		super(ureq, wControl);
		// Read current configuration
		this.displayConfig = config.getStringValue(BBautOLATStructureNodeEditController.CONFIG_KEY_DISPLAY_TYPE,
				BBautOLATStructureNodeEditController.CONFIG_VALUE_DISPLAY_TOC);
		this.columnsConfig = config.getIntegerSafe(BBautOLATStructureNodeEditController.CONFIG_KEY_COLUMNS, 1);
	...
	protected void initForm(FormItemContainer formLayout, Controller listener,
		...
		} else if (displayConfig.equals(BBautOLATStructureNodeEditController.CONFIG_VALUE_DISPLAY_DELEGATE)) {
		...
		if (selectedPeekviewChildKeys.length > 0) {
			...
			selectedPeekviewChildren.setLabel("selectedPeekviewChildren",
					new String[] { BBautOLATStructureNodeConfiguration.MAX_PEEKVIEW_CHILD_NODES + "" });
		...
		if (displayConfig.equals(BBautOLATStructureNodeEditController.CONFIG_VALUE_DISPLAY_FILE) || displayConfig.equals(BBautOLATStructureNodeEditController.CONFIG_VALUE_DISPLAY_DELEGATE)) {
	...
	protected void formInnerEvent(UserRequest ureq, FormItem source, FormEvent event) {
		...
		if (source == selectedPeekviewChildren) {
			...
			if (selectedPeekviewChildren.getSelectedKeys().size() >= BBautOLATStructureNodeConfiguration.MAX_PEEKVIEW_CHILD_NODES) {
				...
				showInfo("form.peekview.max.reached", BBautOLATStructureNodeConfiguration.MAX_PEEKVIEW_CHILD_NODES + "");
			} else {
	...
	private void preselectConfiguredOrMaxChildNodes() {
		...
		if (selectedPeekviewChildren != null) {
			...
			if (selectedPeekviewChildren.getSelectedKeys().size() == 0) {
				for (int i = 0; i < selectedPeekviewChildKeys.length; i++) {
					if (i < BBautOLATStructureNodeConfiguration.MAX_PEEKVIEW_CHILD_NODES) {
	...
	public void updateModuleConfiguration(ModuleConfiguration moduleConfig) {
		...
		if (BBautOLATStructureNodeEditController.CONFIG_VALUE_DISPLAY_FILE.equals(displayType)) {
			// manual file view selected, remove columns config
			moduleConfig.setStringValue(BBautOLATStructureNodeEditController.CONFIG_KEY_DISPLAY_TYPE, BBautOLATStructureNodeEditController.CONFIG_VALUE_DISPLAY_FILE);
			...
			if (BBautOLATStructureNodeEditController.CONFIG_VALUE_DISPLAY_PEEKVIEW.equals(displayType)) {
				moduleConfig.setStringValue(BBautOLATStructureNodeEditController.CONFIG_KEY_DISPLAY_TYPE,
						BBautOLATStructureNodeEditController.CONFIG_VALUE_DISPLAY_PEEKVIEW);
				// update selected peekview children
				if (selectedPeekviewChildren == null || selectedPeekviewChildren.getSelectedKeys().size() == 0) {
					moduleConfig.remove(BBautOLATStructureNodeEditController.CONFIG_KEY_PEEKVIEW_CHILD_NODES);
				} else {
					...
					moduleConfig.set(BBautOLATStructureNodeEditController.CONFIG_KEY_PEEKVIEW_CHILD_NODES, selectedPeekviewChildNodesConfig);
				}
			} else if (BBautOLATStructureNodeEditController.CONFIG_VALUE_DISPLAY_DELEGATE.equals(displayType)) {
					moduleConfig.setStringValue(BBautOLATStructureNodeEditController.CONFIG_KEY_DISPLAY_TYPE,
						BBautOLATStructureNodeEditController.CONFIG_VALUE_DISPLAY_DELEGATE);
			} else {
				// the old auto generated TOC view without peekview
				moduleConfig
						.setStringValue(BBautOLATStructureNodeEditController.CONFIG_KEY_DISPLAY_TYPE, BBautOLATStructureNodeEditController.CONFIG_VALUE_DISPLAY_TOC);
				// Let other config values from old config setup remain in config, maybe
				// used when user switches back to other config (OLAT-5610)
			}
			// in both cases, also set the columns configuration
			int cols = (displayTwoColumns.isSelected(0) ? 2 : 1);
			moduleConfig.setIntValue(BBautOLATStructureNodeEditController.CONFIG_KEY_COLUMNS, Integer.valueOf(cols));
			// Let other config values from old config setup remain in config, maybe
			// used when user switches back to other config (OLAT-5610)
		}
	}
