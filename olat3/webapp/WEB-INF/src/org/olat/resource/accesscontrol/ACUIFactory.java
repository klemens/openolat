...
public class ACUIFactory {
	...
	public static Controller createAccessController(UserRequest ureq, WindowControl wControl,
			List<OfferAccess> links) {
		
		if(links.size() == 1) {
			OfferAccess link = links.get(0);
			...
			if(handler != null) {
				return handler.createAccessController(ureq, wControl, link, null);
			}
		} else if (links.size() > 1) {
			//MainAccessController multiAccessCtrl = new MainAccessController(ureq, wControl, links);
	...
	public static AbstractConfigurationMethodController createAccessConfigurationController(UserRequest ureq, WindowControl wControl,
		...
		AccessMethodHandler handler = module.getAccessMethodHandler(link.getMethod().getType());
		if(handler != null) {
			return handler.createConfigurationController(ureq, wControl, link);
		}
		return null;
	}
