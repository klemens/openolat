...
public class MailerTest extends OlatTestCase {
	...
	private Identity createIdentity(String login) {
		...
		id.getUser().setProperty(UserConstants.EMAIL, login + "olattest@" + maildomain);
		id.getUser().setProperty(UserConstants.LASTNAME, login + "last");
		id.getUser().setProperty(UserConstants.FIRSTNAME, login + "first");
		return id;
	}
	...
	@Test public void testMailAttachments() {
		...
		File file1, file2;
		try {
			System.out.println("MailTest.testMailAttachments Url1=" + MailerTest.class.getResource("MailTest.class") );
			file1 = new File(MailerTest.class.getResource("MailTest.class").toURI());
			attachments[0] = file1;
